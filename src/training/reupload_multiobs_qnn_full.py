...
from src.models.quantum import ResidualDataReuploadingMultiObservableQNNClassifier
...
def run(config_path: str | Path) -> None:
    ...
    started = time.time()
    model = ResidualDataReuploadingMultiObservableQNNClassifier(
    ...
    row = {
        ...
        "model": "residual_reupload_multiobs_qnn_full",
        "qubits": int(qnn["qubits"]),
        "layers": int(qnn["layers"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run full Indian Pines reuploading multi-observable QNN evaluation.")
